...
import { ArrowLeft, Camera, X, ShieldCheck, PawPrint, AlertTriangle } from 'lucide-react'
import Link from 'next/link'
import { updatePet, getPet, deletePet } from '@/app/actions/pets'
...
export default function EditPetPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (photos.length === 0) {
      toast.error('You must retain or capture at least one live verification photo.')
      return
    }
    setSaving(true)
  ...
  const handleDelete = async () => {
    if (!confirm('Are you absolutely sure you want to permanently remove this companion?')) return
    setSaving(true)
    const result = await deletePet(id as string)
    if (result.success) {
      toast.success(result.success)
      router.push('/profile')
    } else {
        ...
        <header className="mb-16 relative">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-10">
            <div>
              ...
              <h1 className="font-serif text-5xl lg:text-6xl font-bold text-[#022717] mb-4 tracking-tight">Update Resident</h1>
              <p className="text-lg text-[#022717]/50 max-w-lg leading-relaxed font-sans italic">
                Revise your companion's file in the elite registry using strict live-camera security.
              </p>
            </div>
              ...
              <section className="space-y-10">
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <Input id="name" name="name" defaultValue={pet?.name} className="h-14 rounded-xl bg-white border border-black/5 shadow-sm text-base px-5 focus:border-[#835500] focus:ring-1 focus:ring-[#835500] transition-all" required />
                  </div>
                  <div className="space-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    <Input id="breed" name="breed" defaultValue={pet?.breed} className="h-14 rounded-xl bg-white border border-black/5 shadow-sm text-base px-5 focus:border-[#835500] focus:ring-1 focus:ring-[#835500] transition-all" />
                  </div>
                  <div className="space-y-3">
                    <Label htmlFor="age_months" className="text-[10px] uppercase font-bold tracking-[0.2em] text-[#022717]/40">Age (Months)</Label>
                    <Input id="age_months" name="age_months" type="number" min="0" defaultValue={pet?.age_months} className="h-14 rounded-xl bg-white border border-black/5 shadow-sm text-base px-5 focus:border-[#835500] focus:ring-1 focus:ring-[#835500] transition-all" required />
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-10">
                ...
                <div className="space-y-8">
                  ...
                  <div className="flex items-center gap-4 bg-white p-6 rounded-2xl border border-black/5 shadow-sm">
                    <Checkbox id="is_vaccinated" name="is_vaccinated" value="true" defaultChecked={pet?.is_vaccinated} className="w-5 h-5 rounded border-black/10 data-[state=checked]:bg-[#022717] data-[state=checked]:border-[#022717]" />
            ...
            <div className="space-y-8">
               <div className="bg-white p-8 rounded-[2rem] ambient-shadow border border-black/5 sticky top-12">
                    ...
                    {isCameraOpen ? (
                      <>
                        ...
                        <div className="absolute bottom-6 left-0 right-0 flex justify-center z-10">
                           <Button type="button" onClick={capturePhoto} disabled={saving} className="rounded-full h-14 px-8 bg-white text-[#022717] hover:bg-gray-100 shadow-xl font-bold text-[10px] uppercase tracking-widest">
                             {saving ? 'Securing...' : 'Capture Image'}
                           </Button>
                        </div>
                  ...
                  <footer className="space-y-4">
                     <Button type="submit" disabled={saving || photos.length === 0} className="w-full h-16 rounded-xl bg-[#022717] text-white text-[11px] font-bold uppercase tracking-[0.3em] shadow-xl hover:bg-[#1a3d2b] transition-all disabled:opacity-30">
                        {saving ? 'Processing...' : 'Save Updates'}
                     </Button>
                     <Button type="button" onClick={handleDelete} disabled={saving} variant="outline" className="w-full h-12 rounded-xl text-red-500 border border-red-500/20 bg-red-50/50 hover:bg-red-50 hover:text-red-700 text-[10px] font-bold uppercase tracking-[0.2em] hover:border-red-500/40 transition-all shadow-sm">
                        <AlertTriangle className="w-4 h-4 mr-2" /> Permanently Delete Resident
                     </Button>
                  </footer>
               </div>
            </div>
